#1
while True:
    units = list(input().split())
    print(eval(units[0].join(units[1:])))

#2
units = input().split()
assert units[0] in '+-*/', 'Invalid string'
try:
    if len(units) > 3:
        raise IndexError
    else:
        print(eval(f'{units[1]}{units[0]}{units[2]}'))
except ZeroDivisionError:
    print("don't")
except TypeError:
    print('del your quotes')
except IndexError:
    print('wrong number of arguments')
except Exception as x:
    print(f'another error called {x}')
